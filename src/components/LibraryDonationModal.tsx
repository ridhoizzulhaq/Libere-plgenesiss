...
interface LibraryDonationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (libraryAddress: string, amount: number, libraryName: string) => void;
  bookPrice?: string;
}
...
const LibraryDonationModal = ({ isOpen, onClose, onConfirm, bookPrice }: LibraryDonationModalProps) => {
  ...
  const handleConfirm = () => {
    ...
    // All valid - proceed
    const libraryName = selectedLibrary === 'custom'
      ? 'Custom Library'
      : libraries.find(lib => lib.address === selectedLibrary)?.name ?? 'Library';
    onConfirm(addressToUse, amount, libraryName);
  };
