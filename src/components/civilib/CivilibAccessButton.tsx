...
const CivilibAccessButton = ({
  ...
  const { recordBorrow } = useHypercerts();
  ...
  const onBorrowBook = async () => {
    ...
    try {
      ...
      // Record borrow measurement ke hypercert (non-blocking, deduplicated by txHash)
      if (recordedTxRef.current !== tx) {
        recordedTxRef.current = tx;
        recordBorrow({
          bookId,
          libraryAddress: effectiveLibraryAddress,
          borrowerWallet: client.account?.address,
          txHash: tx,
        }).catch(console.warn);
      }
